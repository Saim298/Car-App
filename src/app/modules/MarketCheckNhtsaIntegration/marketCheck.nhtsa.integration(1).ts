...
export const getMarketCheckData = async (userId: string): Promise<any> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const query = `
      SELECT * FROM market_check_vehicle_specs WHERE user_id = ?;
    `;
    db.query(query, [userId], (err, results: any[]) => {
      if (err) {
        console.error('Step 1 Error: Querying market_check_vehicle_specs', err);
        return reject(err);
      }
      console.log(
        'Step 2: Received market_check_vehicle_specs results:',
        results
      );
      if (!results.length) {
        console.log(
          'Step 3: No market_check_vehicle_specs found. Returning null.'
        );
        return resolve(null);
      }

      const marketCheckData = results[0];
      console.log(
        'Step 4: Using first market_check_vehicle_specs record:',
        marketCheckData
      );

      const nestedQueries = {
        listings: `SELECT * FROM market_place_listings WHERE user_id = ?;`,
        listingDealers: `SELECT * FROM market_place_listing_dealers WHERE listing_id IN (SELECT id FROM market_place_listings WHERE vin = ?);`,
        vehicleMedia: `SELECT * FROM market_place_vehicle_media WHERE vin = ?;`,
        nhtsaSafetyRating: `SELECT * FROM nhtsa_safety_rating WHERE user_id = ?;`,
        nhtsaRecalls: `SELECT * FROM nhtsa_recalls WHERE user_id = ?;`,
        nhtsaComplaints: `SELECT * FROM nhtsa_complaints WHERE user_id = ?;`,
      };

      console.log(
        'Step 5: Querying market_place_listings for user_id:',
        userId
      );
      db.query(nestedQueries.listings, [userId], (err, listings: any[]) => {
        if (err) {
          console.error('Step 5 Error: Querying market_place_listings', err);
          return reject(err);
        }
        ...
        if (!marketCheckData.vin) {
          console.log(
            'Step 7: No VIN found in market_check_vehicle_specs. Resolving with current marketCheckData:',
            marketCheckData
          );
          return resolve(marketCheckData);
        }
        ...
        db.query(
          nestedQueries.listingDealers,
          [marketCheckData.vin],
          (err, listingDealers: any[]) => {
            if (err) {
              console.error(
            ...
            // Step 10: Extract dealer IDs from listingDealers
            const dealerIds = listingDealers.map((ld: any) => ld.dealer_id);
            ...
            if (dealerIds.length) {
              ...
              db.query(
                `SELECT * FROM market_place_dealers WHERE id IN (?);`,
                [dealerIds],
                (err, dealers: any[]) => {
                  if (err) {
                    console.error(
                  ...
                  // Step 13: Map dealer details to listingDealers
                  const dealerMap = new Map();
                  dealers.forEach((dealer: any) => {
                    dealerMap.set(dealer.id, dealer);
                  });

                  // Step 14: Attach dealer details to listingDealers
                  ...
                  // Step 15: Query market_place_vehicle_media for VIN
                  console.log(
                    'Step 15: Querying market_place_vehicle_media for VIN:',
                    marketCheckData.vin
                  );
                  db.query(
                    nestedQueries.vehicleMedia,
                    [marketCheckData.vin],
                    (err, mediaResults: any[]) => {
                      if (err) {
                        console.error(
                      ...
                      console.log(
                        'Step 16: Received media results:',
                        mediaResults
                      );

                      // Step 17: Organize media into photo_links and video_links
                      const media = {
                      ...
                      // Step 19: Query NHTSA data
                      console.log(
                        'Step 19: Querying NHTSA safety rating for user_id:',
                        userId
                      );
                      db.query(
                        nestedQueries.nhtsaSafetyRating,
                        [userId],
                        (err, safetyRatingResults: any[]) => {
                          if (err) {
                            console.error(
                          ...
                          console.log(
                            'Step 21: Querying NHTSA recalls for user_id:',
                            userId
                          );
                          db.query(
                            nestedQueries.nhtsaRecalls,
                            [userId],
                            (err, recallResults: any[]) => {
                              if (err) {
                                console.error(
                              ...
                              console.log(
                                'Step 22: Received NHTSA recall results:',
                                recallResults
                              );
                              ...
                              db.query(
                                nestedQueries.nhtsaComplaints,
                                [userId],
                                (err, complaintResults: any[]) => {
                                  if (err) {
                                    console.error(
                                  ...
                                  // Step 25: Construct NHTSA data object
                                  const nhtsaData = {
                                    safety_ratings:
                                      safetyRatingResults[0] || null,
                                    recalls: recallResults,
                                    complaints: complaintResults,
                                  };

                                  // Step 26: Construct final response
                                  const response = {
                                    marketCheckData: {
                                      ...
                                      price:
                                        marketCheckData.listings[0]?.price ||
                                        null,
                                      miles:
                                        marketCheckData.listings[0]?.miles ||
                                        null,
                                      exterior_color:
                                        marketCheckData.exterior_color,
                                      interior_color:
                                        marketCheckData.interior_color,
                                      media,
                                      dealer:
...
export const getMarketCheckData_2 = async (
  ...
  return new Promise((resolve, reject) => {
    console.log(
      'Step 1: Querying market_check_vehicle_specs for user_id and VIN:',
      userId,
      vin
    );
    const query = `
      SELECT * FROM market_check_vehicle_specs WHERE user_id = ? AND vin = ?;
    `;

    db.query(query, [userId, vin], (err, results: any[]) => {
      if (err) {
        console.error('Step 1 Error: Querying market_check_vehicle_specs', err);
        return reject(err);
      }
      ...
      if (!results || results.length === 0) {
        console.log(
          'Step 3: No market_check_vehicle_specs found for the given user_id and VIN.'
        );
        return resolve(null);
      }
      ...
      const marketCheckData = results[0];
      console.log(
        'Step 4: Using market_check_vehicle_specs record:',
        marketCheckData
      );

      const nestedQueries = {
        listings: `SELECT * FROM market_place_listings WHERE user_id = ? AND vin = ?;`,
        listingDealers: `SELECT * FROM market_place_listing_dealers WHERE listing_id IN (SELECT id FROM market_place_listings WHERE vin = ?);`,
        vehicleMedia: `SELECT * FROM market_place_vehicle_media WHERE vin = ?;`,
        nhtsaSafetyRating: `SELECT * FROM nhtsa_safety_rating WHERE vin = ?;`,
        nhtsaRecalls: `SELECT * FROM nhtsa_recalls WHERE vin = ?;`,
        nhtsaComplaints: `SELECT * FROM nhtsa_complaints WHERE vin = ?;`,
      };
      ...
      db.query(
        ...
        (err, listings: any[]) => {
          ...
          console.log(
            'Step 7: Querying market_place_listing_dealers for VIN:',
            vin
          );
          db.query(
            nestedQueries.listingDealers,
            ...
            (err, listingDealers: any[]) => {
              ...
              // Step 9: Extract dealer IDs from listingDealers
              const dealerIds = listingDealers.map((ld: any) => ld.dealer_id);
              ...
              if (dealerIds.length) {
                ...
                db.query(
                  ...
                  (err, dealers: any[]) => {
                    ...
                    // Step 12: Map dealer details to listingDealers
                    const dealerMap = new Map();
                    dealers.forEach((dealer: any) => {
                      dealerMap.set(dealer.id, dealer);
                    });

                    // Step 13: Attach dealer details to listingDealers
                    const listingDealersWithDealers = listingDealers.map(
                      (ld: any) => ({
                        ...ld,
                        dealer: dealerMap.get(ld.dealer_id) || null,
                    ...
                    // Step 14: Query market_place_vehicle_media for VIN
                    console.log(
                    ...
                    db.query(
                      ...
                      (err, mediaResults: any[]) => {
                        ...
                        console.log(
                          'Step 15: Received media results:',
                          mediaResults
                        );

                        // Step 16: Organize media into photo_links and video_links
                        const media = {
                          photo_links: mediaResults
                            .filter((m: any) => m.media_type === 'photo')
                            .map((m: any) => m.media_url),
                        ...
                        // Step 18: Query NHTSA data
                        console.log(
                          'Step 18: Querying NHTSA safety rating for vin:',
                          vin
                        );
                        db.query(
                          nestedQueries.nhtsaSafetyRating,
                          ...
                          (err, safetyRatingResults: any) => {
                            ...
                            console.log(
                              'Step 20: Querying NHTSA recalls for vin:',
                              vin
                            );
                            db.query(
                              nestedQueries.nhtsaRecalls,
                              ...
                              (err, recallResults: any[]) => {
                                ...
                                console.log(
                                  'Step 21: Received NHTSA recall results:',
                                  recallResults
                                );

                                console.log(
                                  'Step 22: Querying NHTSA complaints for vin:',
                                  vin
                                );
                                db.query(
                                  nestedQueries.nhtsaComplaints,
                                  ...
                                  (err, complaintResults: any[]) => {
                                    ...
                                    // Step 24: Construct NHTSA data object
                                    const nhtsaData = {
                                    ...
                                    // Step 25: Construct final response
                                    const response = {
                                      marketCheckData: {
                                        id: marketCheckData.id,
                                        vin: marketCheckData.vin,
...
export const getMarketCheckData_1 = async (userId: string): Promise<any> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    db.query(query, [userId], (err, results: any[]) => {
      ...
      const nestedQueries = {
        listings: `SELECT * FROM market_place_listings WHERE user_id = ?;`,
        dealers: `SELECT * FROM market_place_dealers WHERE id IN (SELECT dealer_id FROM market_place_dealer_search WHERE user_id = ?);`,
        listingDealers: `SELECT * FROM market_place_listing_dealers WHERE vin = ?;`,
      };
      ...
      db.query(nestedQueries.listings, [userId], (err, listings: any[]) => {
        ...
        console.log(
          'Step 7: Querying market_place_dealers for user_id:',
          userId
        );
        db.query(nestedQueries.dealers, [userId], (err, dealers: any[]) => {
          if (err) {
            console.error('Step 7 Error: Querying market_place_dealers', err);
            return reject(err);
          }
          ...
          if (!marketCheckData.vin) {
            console.log(
              'Step 9: No VIN found in market_check_vehicle_specs. Resolving with current marketCheckData:',
              marketCheckData
            );
            return resolve(marketCheckData);
          }
...
export const getMarketCheckData_vin = async (userId: string): Promise<any> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const query = `SELECT vin FROM market_check_vehicle_specs WHERE user_id = ?;`;
    ...
    db.query(query, [userId], async (err, results: any[]) => {
      ...
      const vinArray = results.map((row) => row.vin);
