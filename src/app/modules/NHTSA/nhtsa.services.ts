...
import { supabase } from '../../config/superbase'; // Import Supabase client
...
export async function decodeVin(vin: string, user_id?: string) {
  try {
    ...
    if (user_id) {
      ...
      console.log('Inserting Data into nhtsa_vin_decode using Supabase:', extractedData);
      
      try {
        // Use Supabase instead of MySQL
        const { data, error } = await supabase
          .from('nhtsa_vin_decode')
          .insert([extractedData]);
        
        if (error) {
          console.error('Error inserting into nhtsa_vin_decode:', error);
          console.error('Error code:', error.code);
          console.error('Error message:', error.message);
          
          // Check for specific error codes
          if (error.code === '42501') {
            console.error('Permission denied error. Make sure RLS is disabled for the nhtsa_vin_decode table.');
            console.error('Run this SQL: ALTER TABLE nhtsa_vin_decode DISABLE ROW LEVEL SECURITY;');
            console.error('Also run: GRANT ALL PRIVILEGES ON TABLE nhtsa_vin_decode TO anon, authenticated;');
          }
        } else {
          console.log('Successfully inserted data into nhtsa_vin_decode');
        }
      } catch (insertError) {
        console.error('Exception inserting into nhtsa_vin_decode:', insertError);
      }
    }
  ...
  try {
    ...
    const vehicleDetails = await Promise.all(
      vehicles.map(async (vehicle: { VehicleId: number }) => {
        ...
        if (user_id) {
          console.log(
            'Inserting Data into nhtsa_safety_rating using Supabase:',
            extractedData
          );
          ...
          try {
            ...
            if (error) {
              console.error('Error inserting into nhtsa_safety_rating:', error);
              console.error('Error code:', error.code);
              console.error('Error message:', error.message);
              
              // Check for specific error codes
              if (error.code === '42501') {
                console.error('Permission denied error. Make sure RLS is disabled for the nhtsa_safety_rating table.');
                console.error('Run this SQL: ALTER TABLE nhtsa_safety_rating DISABLE ROW LEVEL SECURITY;');
                console.error('Also run: GRANT ALL PRIVILEGES ON TABLE nhtsa_safety_rating TO anon, authenticated;');
              }
            } else {
              console.log('Successfully inserted data into nhtsa_safety_rating');
            }
          } catch (insertError) {
            console.error('Exception inserting into nhtsa_safety_rating:', insertError);
          }
        }
...
export async function getRecallsByMMY(
  ...
  try {
    ...
    for (const recall of formattedRecalls) {
      try {
        console.log('Inserting recall into nhtsa_recalls using Supabase:', recall);
        ...
        if (error) {
          console.error('Error inserting into nhtsa_recalls:', error);
          console.error('Error code:', error.code);
          console.error('Error message:', error.message);
          
          // Check for specific error codes
          if (error.code === '42501') {
            console.error('Permission denied error. Make sure RLS is disabled for the nhtsa_recalls table.');
            console.error('Run this SQL: ALTER TABLE nhtsa_recalls DISABLE ROW LEVEL SECURITY;');
            console.error('Also run: GRANT ALL PRIVILEGES ON TABLE nhtsa_recalls TO anon, authenticated;');
          }
        } else {
          console.log('Successfully inserted recall into nhtsa_recalls');
        }
      } catch (insertError) {
        console.error('Exception inserting recall into nhtsa_recalls:', insertError);
      }
    }

    console.log(
      `Processed ${formattedRecalls.length} recalls for insertion into nhtsa_recalls`
    );
...
export async function insertRecallsIntoDB(user_id: string, recalls: any[]) {
  ...
  try {
    ...
    console.log('Recalls successfully processed for insertion into nhtsa_recalls');
  } catch (error: any) {
...
export async function getComplaintsByMMY(
  ...
  try {
    ...
    // Check how many complaints already exist for this VIN using Supabase
    ...
    if (countError) {
      console.error('Error checking existing complaints:', countError);
      return;
    }

    const existingCount = existingComplaints ? existingComplaints.length : 0;
    ...
    for (const complaint of batchToInsert) {
      try {
        console.log('Inserting complaint into nhtsa_complaints using Supabase:', complaint);
        ...
        if (error) {
          console.error('Error inserting into nhtsa_complaints:', error);
          console.error('Error code:', error.code);
          console.error('Error message:', error.message);
          
          // Check for specific error codes
          if (error.code === '42501') {
            console.error('Permission denied error. Make sure RLS is disabled for the nhtsa_complaints table.');
            console.error('Run this SQL: ALTER TABLE nhtsa_complaints DISABLE ROW LEVEL SECURITY;');
            console.error('Also run: GRANT ALL PRIVILEGES ON TABLE nhtsa_complaints TO anon, authenticated;');
          }
        } else {
          console.log('Successfully inserted complaint into nhtsa_complaints');
        }
      } catch (insertError) {
        console.error('Exception inserting complaint into nhtsa_complaints:', insertError);
      }
    }

    console.log(
      `Processed ${batchToInsert.length} complaints for VIN ${vin} (Total now: ${
